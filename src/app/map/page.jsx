...
const MapComponent = () => {
  ...
  return (
    <>
      ...
      {nearestLocations.map((location) => (
      <Alert key={location.id} onClick={() => handleLocationClick(location)} className="my-2">
        <Terminal className="h-4 w-4" />
        <div className="gap-2 flex ">
        <AlertTitle className="m-0 pt-1" >{location.name}</AlertTitle>
        <Badge className="ml-2" style={{backgroundColor: location.color}}>{location.distance.toFixed(3)}{" "}km</Badge>
        </div>
        <AlertDescription>
          Estimated time: {calculateEstimatedTime(location.distance)} minutes
        </AlertDescription>
      </Alert>
      ))}
      </div>
